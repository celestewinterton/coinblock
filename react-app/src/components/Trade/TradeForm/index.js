...
const TradeForm = ({ showModal, setShowModal, preSetId }) => {
  ...
  const ownedCoins = Object.values(coins).filter((coin) =>
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append(
      "limit",
      type === "buy" ? user.balances.cash : user.balances[cryptoId] * price
    );
    ...
    <div className="transaction-form-container column">
      <div className="row">
        <NavLink
          ...
          className="buy-tab"
          style={
            type === "buy" ? { color: "#0052FF", borderBottom: "none" } : null
          }
        ...
        <NavLink
          ...
          className="sell-tab"
          style={
            type === "sell" ? { color: "#0052FF", borderBottom: "none" } : null
          }
        ...
        <NavLink
          ...
          style={
            type === "transfer"
              ? { color: "#0052FF", borderBottom: "none" }
              : null
          }
          onClick={(e) => setType("transfer")}
        >
          Add Funds
        </NavLink>
      </div>
      <form
        className="center column trade-form"
        autoComplete="off"
      ...
      >
        <div className="amount-input-container">
          <label className="row amount-label">
            ...
            <input
              ...
              style={
                amount.length
                  ? { width: `${26 * amount.length}px`, maxWidth: "182px" }
                  : { width: "24px" }
              }
              required
          ...
          <div className="form-errors">
            {errors ? <p>{errors.amount}</p> : null}
          </div>
        </div>
        <div className="muted1">
          {type === "transfer"
            ? null
            : `1 ${coins[cryptoId]?.symbol.toUpperCase()} ≈ ${currency(price)}`}
        </div>
        <div className="muted1">
          Your current cash balance is {currency(user.balances.cash)}
        </div>
        <div className="select-inputs">
          {type !== "transfer" && (
            <div className="select-field">
              <label className="muted1">
                {type === "buy" ? "Buy" : "Sell"}
              </label>
              <select
                type="text"
              ...
              >
                {type === "buy" ? (
                  ...
                  Object.values(ownedCoins).map((coin) => (
                    <option key={coin.id} value={coin.id}>
                      {coin.name}
                    </option>
                  ))
          ...
          {type === "transfer" && (
            <div className="select-field">
              ...
              >
                <option key="bank1" value="Bank of Satoshi">
                  Bank of Satoshi
                </option>
                <option key="bank2" value="Bank of America">
                  Bank of America
                </option>
                <option key="bank3" value="Chase">
                  Chase
                </option>
              </select>
            </div>
        ...
        <button className="wide top-margin large-button" type="submit">
          {type === "transfer"
            ? "Deposit Funds"
            : type === "buy"
            ? `Buy ${coins[cryptoId]?.name}`
            : `Sell ${coins[cryptoId]?.name}`}
        </button>
        {type !== "transfer" && (
          <div className="top-margin muted1 row space-between">
            <div>{coins[cryptoId]?.symbol.toUpperCase()} balance</div>
            <div>
              {user.balances[cryptoId] ? round(user.balances[cryptoId]) : 0}{" "}
              {coins[cryptoId]?.symbol.toUpperCase()} ≈{" "}
              {user.balances[cryptoId]
                ? currency(user.balances[cryptoId] * price)
                : "$0"}
            </div>
          </div>
        )}
