  ...
  return (
    <>
      <div className="row space-between">
        ...
        {apiIds.length > 0 && (
          <button className="muted-button top-margin">
            <NavLink to={`/trade`} className="">
              See all assets <i class="fa-solid fa-arrow-right"></i>
            </NavLink>
          </button>
        )}
      </div>
      {!apiIds.length && (
        <div className="padded">
          <div className="bold2">
            Looks like you don't have anything on your watchlist.
          </div>
          <div className="padded">
            <NavLink to={`/trade`} className="button top-margin">
          ...
          {apiIds.map((apiId, idx) => (
            <NavLink to={`/${apiId}`}>
              <tr>
                ...
                <td>
                  {change(data[apiId]?.high_24h, data[apiId]?.current_price)}
                </td>
                <td>${bigNum(data[apiId]?.market_cap)}</td>
                <td>
                  <button
                    id={
                      watchlist.find((record) => record.crypto.apiId === apiId)
                        .id
                    }
                  ...
                  >
                    <i
                      ...
                      id={
                        watchlist.find(
                          (record) => record.crypto.apiId === apiId
                        ).id
                      }
